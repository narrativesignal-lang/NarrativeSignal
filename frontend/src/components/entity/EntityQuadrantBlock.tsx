...
export function EntityQuadrantBlock({
  ...
  const points = useMemo(() => {
    ...
    return p.map((x) => ({
      t: x.t,
      coverage_volume: x.coverage_volume,
      keywords_search_volume: x.keywords_search_volume,
    }));
  }, [histQuery.data]);

  const env = histQuery.data;
  const showPrep =
  ...
  const error = histQuery.isError
    ? (histQuery.error as { message?: string })?.message ?? "Failed to load quadrant"
    : null;

  if (loading) {
    return <BlockStateMessage kind="loading" height={200} />;
  }
  if (error) {
    return (
    ...
    <div className="flex h-full min-h-0 min-w-0 flex-col gap-2">
      ...
      {showStale && env?.data_updated_at ? (
        <div className="text-[10px] text-slate-500">
          <span className="text-amber-200/80">Stale</span> · {new Date(env.data_updated_at).toLocaleString()}
        </div>
      ) : null}
      <QuadrantChart points={points ?? undefined} periodLabel={period} />
    </div>
  );
}
